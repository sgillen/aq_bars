...
int main(int argc, char* argv[])
{
    ...
    if(init_falcon(0))
		{ 
			...
			ros::Subscriber sub = node.subscribe("/falconForce", 10, &forceCallback,  ros::TransportHints().tcpNoDelay());
        
			//Start ROS Publisher
			ros::Publisher pub = node.advertise<aqbar::falconPos>("falconPos",10);        

			ros::Rate loop_rate(10);

			std::array<double,3> forces;
			aqbar::falconPos position_msg;
					...
					if(g_falconDevice.runIOLoop())
						{
						  ...
						  position_msg.X = g_pos[0];
						  position_msg.Y = g_pos[1];
						  position_msg.Z = g_pos[2];
